...
void AutoTestWindow::on_connectInstrumentButton_clicked()
{
    ...
    appendInstrumentLog(QString("[%1] 仪器已连接：%2")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
                       .arg(visaAddr));
}
...
void AutoTestWindow::on_disconnectInstrumentButton_clicked()
{
    ...
    m_instrumentConnected = false;
    appendInstrumentLog(QString("[%1] 仪器已断开")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
}
...
void AutoTestWindow::on_initInstrumentButton_clicked()
{
    ...
    appendInstrumentLog(QString("[%1] 正在初始化仪器...")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));

    // TODO: 实现实际的仪器初始化 SCPI 指令

    appendInstrumentLog(QString("[%1] 仪器初始化完成")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
}
...
void AutoTestWindow::on_readParamButton_clicked()
{
    ...
    appendInstrumentLog(QString("[%1] 正在读取仪器参数...")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));

    // TODO: 实现实际的参数读取 SCPI 指令

    appendInstrumentLog(QString("[%1] 参数读取完成")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
}
...
void AutoTestWindow::on_setParamButton_clicked()
{
    ...
    appendInstrumentLog(QString("[%1] 参数设置完成")
                       .arg(QDateTime::currentDateTime().toString("hh:mm:ss")));
}
