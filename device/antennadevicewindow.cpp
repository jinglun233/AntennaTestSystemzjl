...
AntennaDeviceWindow::AntennaDeviceWindow(QWidget *parent)
    ...
    , m_telemetryCsvFile(nullptr)
    , m_telemetryCsvStream(nullptr)
{
    ui->setupUi(this);
...
{
    // 关闭并释放 CSV 文件资源
    if (m_telemetryCsvStream) {
        m_telemetryCsvStream->flush();
        delete m_telemetryCsvStream;
        m_telemetryCsvStream = nullptr;
    }
    if (m_telemetryCsvFile) {
        m_telemetryCsvFile->close();
        delete m_telemetryCsvFile;
        m_telemetryCsvFile = nullptr;
    }
    delete ui;
}
...
void AntennaDeviceWindow::updateWaveControlTelemetry(int waveId, const QVector<QString> &values)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (item) {
            ...
            // 数字遥测（第15项，索引14）不为 0x000000 时标红底白字
            if (i == 14 && values.at(14) != QLatin1String("0x000000")) {
                item->setBackground(QColor("#e74c3c"));
                item->setForeground(QColor("#ffffff"));
            }
            // 注意：非标红行不修改背景/前景色，保持原始交替行颜色不变
        }
    }
}
...
void AntennaDeviceWindow::parseWaveControlTelemetry(const QByteArray &payload)
{
    ...
    // 收集全部 12 路波控遥测数据，用于 CSV 保存
    QVector<QVector<QString>> allWaveValues;
    allWaveValues.reserve(12);
    ...
    for (int waveId = 1; waveId <= 12; waveId++) {
        ...
        // 收集到 CSV 数据容器
        allWaveValues.append(values);
    }

    // 将完整的 12 路遥测数据保存到 CSV 文件
    saveTelemetryToCsv(allWaveValues);
}
...
QStringList AntennaDeviceWindow::buildTelemetryColumnNames()
{
    QStringList cols;
    cols.reserve(181);  // 时间 + 12×15 = 1 + 180
    cols.append(QStringLiteral("时间"));

    // 波控 1~12，每路 15 个遥测量名称（与 initTelemetryTables 中 names1~names12 一致）
    const QString items[] = {
        QStringLiteral("+5V1"), QStringLiteral("-5V1"), QStringLiteral("+5V2"), QStringLiteral("-5V2"),
        QStringLiteral("28V1"), QStringLiteral("28V2"),
        QStringLiteral("TR组件1"), QStringLiteral("TR组件2"), QStringLiteral("TR组件3"), QStringLiteral("TR组件4"),
        QStringLiteral("TR组件5"), QStringLiteral("TR组件6"), QStringLiteral("TR组件7"), QStringLiteral("TR组件8"),
        QStringLiteral("数字遥测")
    };

    for (int w = 1; w <= 12; ++w) {
        QString prefix = QString("波控%1 ").arg(w);
        for (int i = 0; i < 15; ++i) {
            cols.append(prefix + items[i]);
        }
    }
    return cols;
}

void AntennaDeviceWindow::ensureTelemetryCsvFileOpen()
{
    // 获取当前日期字符串 yyyyMMdd
    QString today = QDateTime::currentDateTime().toString(QStringLiteral("yyyyMMdd"));

    // 如果日期未变且文件已打开，直接复用
    if (m_telemetryCsvFile && m_currentCsvDate == today && m_telemetryCsvFile->isOpen()) {
        return;
    }
    ...
    // 在桌面创建"天线遥测数据"文件夹（不存在则自动创建）
    QString desktopPath = QStandardPaths::writableLocation(QStandardPaths::DesktopLocation);
    QString dirPath = desktopPath + QStringLiteral("/天线遥测数据");
    if (!QDir(dirPath).exists()) {
        QDir().mkpath(dirPath);  // 递归创建所有父目录
    }

    // 构建文件名：yyyyMMdd_遥测数据.csv
    QString fileName = today + QStringLiteral("_遥测数据.csv");
    QString filePath = dirPath + QLatin1Char('/') + fileName;

    // 打开/创建文件（追加模式）
    m_telemetryCsvFile = new QFile(filePath);
    QIODevice::OpenMode openMode = QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text;
    bool isNewFile = !QFile::exists(filePath);  // 是否需要写表头

    if (!m_telemetryCsvFile->open(openMode)) {
        qWarning() << "无法打开遥测CSV文件:" << filePath << m_telemetryCsvFile->errorString();
        delete m_telemetryCsvFile;
        m_telemetryCsvFile = nullptr;
        return;
    }

    m_telemetryCsvStream = new QTextStream(m_telemetryCsvFile);
    m_telemetryCsvStream->setCodec("UTF-8");

    // 新文件：先写入 UTF-8 BOM 原始字节（直接写文件，不经过 QTextStream 编码），再写表头
    if (isNewFile) {
        m_telemetryCsvFile->write("\xEF\xBB\xBF");  // UTF-8 BOM 原始字节
        QStringList headers = buildTelemetryColumnNames();
        *m_telemetryCsvStream << headers.join(QLatin1Char(',')) << "\n";
        m_telemetryCsvStream->flush();
    }

    m_currentCsvDate = today;
}

void AntennaDeviceWindow::saveTelemetryToCsv(const QVector<QVector<QString>> &allWaveValues)
{
    if (allWaveValues.size() != 12) return;

    // 确保 CSV 文件已打开
    ensureTelemetryCsvFileOpen();
    if (!m_telemetryCsvStream || !m_telemetryCsvFile || !m_telemetryCsvFile->isOpen()) {
        return;
    }

    // 构建一行数据：时间 + 12路×15列 = 181 列
    QString line;
    line.reserve(2048);
    ...
    // 追加每路的 15 个值
    for (const QVector<QString> &waveVals : allWaveValues) {
        for (const QString &val : waveVals) {
            line += QLatin1Char(',');
            line += val;  // 值本身不含逗号和换行，无需额外转义
        }
    }

    *m_telemetryCsvStream << line << "\n";
    m_telemetryCsvStream->flush();
}
