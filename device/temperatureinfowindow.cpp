...
void TemperatureInfoWindow::initCurveContainer()
{
    ...
    // ★★★ 通道选择 LineEdit：回车或失焦时解析通道号并刷新曲线 ★★★
    connect(ui->lineEdit_selectChannel, &QLineEdit::editingFinished,
            this, &TemperatureInfoWindow::onSelectChannelTextChanged);
    // 默认显示全部通道（空集合 = 全选模式）
...
void TemperatureInfoWindow::updateCurves()
{
    ...
    // ★★★ 根据通道选择决定哪些 graph 显示数据/隐藏 ★★★
    bool selectAll = m_selectedChannels.isEmpty();
    for (int ch = 0; ch < TEMP_CHANNEL_COUNT; ++ch) {
        QCPGraph *graph = m_customPlot->graph(ch);
        int channelNum = ch + 1;  // 1-based

        if (selectAll || m_selectedChannels.contains(channelNum)) {
            // 选中的通道：显示数据
            graph->setData(m_timeStamps, m_timeSeries[ch]);
    ...
    // ★★★ 复用上面的 selectAll 变量（已在第316行声明），此处不再重复声明 ★★★
    for (int ch = 0; ch < TEMP_CHANNEL_COUNT; ++ch) {
        // 只计算可见通道的 Y 轴范围
        if (!selectAll && !m_selectedChannels.contains(ch + 1)) continue;
...
/**
 * @brief 通道选择 LineEdit 文本变化回调（回车/失焦触发）
 *
 * 解析用户输入的通道号文本（格式: "1-20,25,40"），
 * 更新 m_selectedChannels，并刷新曲线显示。
 */
void TemperatureInfoWindow::onSelectChannelTextChanged()
{
    QString text = ui->lineEdit_selectChannel->text().trimmed();

    // 空文本或 "all" / "全部" → 显示所有通道
    if (text.isEmpty() || text.compare("all", Qt::CaseInsensitive) == 0
        || text == QString::fromUtf8("全部")) {
        m_selectedChannels.clear();
        for (int ch = 0; ch < TEMP_CHANNEL_COUNT; ++ch) {
            m_customPlot->graph(ch)->setVisible(true);
        }
        m_customPlot->replot();
        return;
    }

    QVector<int> channels = parseChannelSelection(text);
    ...
    // 更新选中集合（用于 updateCurves 过滤）— Qt5 兼容：逐个 insert
    m_selectedChannels.clear();
    for (int ch : channels) {
        m_selectedChannels.insert(ch);
    }

    // 立即刷新曲线显示（根据当前缓存数据重新绘制）
    for (int ch = 0; ch < TEMP_CHANNEL_COUNT; ++ch) {
        int channelNum = ch + 1;
        QCPGraph *graph = m_customPlot->graph(ch);
        if (m_selectedChannels.contains(channelNum)) {
            graph->setVisible(true);
            graph->setData(m_timeStamps, m_timeSeries[ch]);
        } else {
            graph->setVisible(false);
 ...
 *   - 单号: "5", "10,20,30"
 *   - 范围: "1-20", "1-20,50-60"
 *   - 混合: "1-20,25,40,80-100"
 *   - 特殊值: "all"/空 → 全选
 *
 * 返回排序去重的通道号列表（1-based），范围限制在 [1, 112]。
 * 解析失败返回空列表。
 */
QVector<int> TemperatureInfoWindow::parseChannelSelection(const QString &text) const
{
    QVector<int> result;
    ...
    for (const QString &segment : segments) {
        QString seg = segment.trimmed();
        if (seg.isEmpty()) continue;

        // 判断是范围 (N-M) 还是单号 (N)
        if (seg.contains('-')) {
            // 范围格式 N-M
            QStringList rangeParts = seg.split('-', QString::SkipEmptyParts);
            if (rangeParts.size() != 2) return {};  // 格式错误

            bool okStart, okEnd;
            int startNum = rangeParts[0].trimmed().toInt(&okStart);
            int endNum   = rangeParts[1].trimmed().toInt(&okEnd);

            if (!okStart || !okEnd) return {};       // 非数字
            if (startNum < 1 || startNum > 112) return {};  // 超出范围
            if (endNum   < 1 || endNum   > 112) return {};
            if (startNum > endNum) return {};         // 起始 > 结束
            ...
            // 单号格式
            bool ok;
            int num = seg.toInt(&ok);
            if (!ok) return {};              // 非数字
            if (num < 1 || num > 112) return {};  // 超出范围
            result.append(num);
        }
    }
...
void TemperatureInfoWindow::onCurveMouseMoved(QMouseEvent *event)
{
    ...
    for (int i = 0; i < TEMP_CHANNEL_COUNT; ++i) {
        QCPGraph *graph = m_customPlot->graph(i);
        // 只在可见且非空的图上查找最近点（未选中的通道已隐藏+数据清空）
        if (!graph || !graph->visible() || graph->data()->isEmpty()) continue;
